...
inline __host__ __device__ int int_floor(double number) {
  return (int) number < 0.0 ? -ceil(fabs(number)) : floor(number);                                                                   
}     
                     
inline __host__ __device__ double round(double number){
 ...
 private:  
  ...
  void initialize() {//this mallocs/cudamallocs our host/device grid_ & grid_deriv_ pointers
    ...
    if(b_derivatives_) {
      ...
      cudaMalloc(&d_grid_deriv_, DIM * grid_size_ * siezeof(double));//need to make a d_grid_deriv_
      if(!grid_deriv_ or !d_grid_deriv) {
